...
from _alan_calc import get_csvdata,pullStockHistory,get_start_end,adjust_fq_days,pqint
from lsi_daily import run_comment_pppscf #- test ONLY
from _alan_calc import ewma_smooth, extrapolate_series,subDict
...
def calc_pppchgs(dfr,zs=0.,stdev=1.,avg=0.,fqx='D'):
	...
	mx = dfr['difday'].max();dfr['sig_difday']= map(lambda x: 1 if x>=mx else 0,dfr['difday'])
	mx = dfr['fit_chg'].abs().max();dfr['sig_fit_chg']= map(lambda x: 1 if abs(x)>=mx else 0,dfr['fit_chg'])
	mx = dfr['act_chg'].abs().max();dfr['sig_act_chg']= map(lambda x: 1 if abs(x)>=mx else 0,dfr['act_chg'])
...
def batch_pppscf(tkLst,opts=None,optx=None,fpTF=False):
	if opts is None:
		opts, _ = opt_pppscf([])
	if optx is not None:
		opts.update(optx)
...
if __name__ == '__main__':
	(options, args)=opt_pppscf(sys.argv)
	if len(args)==0 or args[0]=='-' :
		tkLst = sys.stdin.read().strip().split("\n")
	else :
		tkLst = args
		options['saveDB']=False
	batch_pppscf(tkLst, options)
