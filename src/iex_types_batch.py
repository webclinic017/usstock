...
def reshape_iex_typx(ticker,typx,jdTmp,tsTF=False,debugTF=False,period='quarterly'):
	...
	if debugTF is True:
		print >> sys.stderr,"typy:{}\ndateColName:{}\ncolLst:{}\n".format(typy,dateColName,colLst)
		print >> sys.stderr,"Before adusted: ",dx.iloc[-1].to_dict()
	...
	try:
		if len(dateColName)>0:
			if 'time' in dateColName[-4:].lower() or 'latestUpdate' in dateColName:
				...
				#dx["pbdate"]=map(lambda x:int(datetime.datetime.fromtimestamp(x/1000).strftime("%Y%m%d")),dx[dateColName])
				#dx["pbdate"]=pbdatetime.apply(lambda x:x.strftime("%Y%m%d"))
				...
				dx["pbdate"]=pbdatetime.apply(lambda x:x.strftime("%Y%m%d")).astype(int)
				#dx["pbdate"]=map(lambda x:int(datetime.datetime.strptime(x,'%Y-%m-%d').strftime("%Y%m%d")),dx[dateColName])
	except Exception as e:
		print >> sys.stderr,"**ERROR: {}, assign pbdate=20010101".format(str(e))
		dx["pbdate"]=20010101
	...
	for ky in set(colLst)-set(dx.columns):
		dx[ky] = None
	if debugTF is True:
		print >> sys.stderr,"After adusted: ",dx.iloc[-1].to_dict()
	if 'close' in colLst:
		dx.dropna(subset=['close'],inplace=True)
	if debugTF:
		print >> sys.stderr,type(dx)
		print >> sys.stderr,colLst
	da = dx[colLst]	
	if debugTF:
		print >> sys.stderr,type(da)
		print >> sys.stderr,da.tail().to_csv(sep='|',index=False)

	if colTyp is not None:
		#print >> sys.stderr,"Assign {} type to {}".format(typx,colTyp)
		try:
			da = da.astype( dict(zip(colLst,colTyp)) )
		except Exception as e:
			print >> sys.stderr,da.head().to_csv(sep='|',index=False)
			for j,(c,t) in enumerate(zip(colLst,colTyp)):
				try:
					...
					print >> sys.stderr, "**ERROR: {} at assigning {}.{}:{} to type {}".format(str(e),j,c,da[c].values,t)
					if t == float:
						da[c]=None
...
def chart2price(typy,dx,saveDB,pgDB,rmode,debugTF=False):
	...
	if debugTF is True and da.shape[0]>0 :
		print >> sys.stderr, "debug @ chart2price():\n",da.tail(2).to_csv(sep='|',index=False)

	if saveDB is True:
		da.to_sql(table1,pgDB,schema='public',index=False,if_exists=rmode)
	else:
		print >> sys.stderr, da.head(2).to_csv(sep='|',index=False)
		print >> sys.stderr, da.tail(2).to_csv(sep='|',index=False)
	return typy
...
def reshape_iex_types(ticker,types,jdTmp,saveDB,pgDB,rmode,debugTF=False,hdrTF=True,output=None,sep='|',indexTF=False,tsTF=False,clientM=None,period='quarterly'):
	...
	for typx in map(str.strip,types.split(',')) :
		try:
			(typy,da)=reshape_iex_typx(ticker,typx,jdTmp,tsTF=tsTF,debugTF=debugTF,period=period)
		except Exception as e:
			print >> sys.stderr, "**ERROR: {} of {}/{} @ reshape_iex_typx()".format(str(e),ticker,typx)
			continue
		if len(da)<1:
			print >> sys.stderr, "**No data for {}/{}".format(ticker,typx)
			continue
		chart2price(typy,da,saveDB,pgDB,rmode,debugTF=debugTF)
		table1="iex_{}_temp".format(typy)
		if debugTF is True:
			print >> sys.stderr, "debug @ reshape_iex_types():\n",da.tail(2).to_csv(sep='|',index=False)
		if saveDB is True:
...
def iex_types_batch(args=[],opt=None,optx=None):
	...
	if debugTF is True:
		...
		print >> sys.stderr, [dbname,hostname,period,saveDB,types,wmode]
	...
	for j,tkTmp  in enumerate(tkM):
		...
		for jk, ticker  in enumerate(tkM[j]):
			...
			try:
				...
				print  >> sys.stderr, str(e), '**ERROR {}. {}'.format(j*n+jk,ticker)
				continue
	if saveDB is False:
		return da, None
	...
	for typx in map(str.strip,types.split(',')) :
		...
		if debugTF:
			print >> sys.stderr,"Update table {} to {}".format(temp,hist)
