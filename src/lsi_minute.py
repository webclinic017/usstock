#!/usr/bin/env python
# -*- coding: utf-8 -*-
""" Program: lsi_minute.py
    Description: create ohlc commentary based on minute_hist and minute_pnl
	...
	def generate_comment_ohlc(f,ts=None,dotSign='.',prcn=0,usdSign='$',udfLst=None,lang="en"):
	...
	def run_comment_ohlc(aX,enhanceX=None,tempS=None,dotSign='.',prcn=0,usdSign='$',lang="cn"):
	def getdb_ohlc_minute(ticker,startYmd,endYmd,sector,label,pgDB):
	...
	def save2mgdb_comment_ohlc(ret,dbM,tablename):
	def iteritem_comment_ohlc(j,tempS,tkX,mndt,mxdt,sector,label,daily_hdr,tablename,pgDB,mgDB,fp,wmode="replace",lang="cn",saveDB=True,mp3YN=False,dirname=None,debugTF=False):
	def assign_ts_ohlc(lang):
	def batch_comment_ohlc(tkLst=None,**kwargs):
	def opt_lsi_minute(argv,retParser=False):
    Last Mod., Fri Aug 17 14:47:09 EDT 2018
"""
from optparse import OptionParser
from _alan_str import *
from _alan_date import delta2dates
from _alan_calc import conn2pgdb,conn2mgdb
...
def getdb_udfLst(category="stock",tbname="mapping_udf_comment",pgDB=None,lang="en"):
	try:
		...
		print  >> sys.stderr, "**ERROR @ getdb_udfLst():",str(e)
		return None
...
def generate_comment_ohlc(f,ts=None,dotSign='.',prcn=0,usdSign='$',udfLst=None,lang="en"):
	...
	if 'enhanceName' in f: # For "enhance" case ONLY
		...
		enhanceName=None
		ynCode=1
	xnDay=delta2dates(f['curr_date'],f['pbdate'],fq="MINUTE") # to calc minutes of previous stragety
	...
	if lang == "cn":
		...
		if xnDay < 1:
			...
			pastDayWd = "前一分鐘".format(xnDay)
		else:
			pastDayWd = "{}分鐘前".format(xnDay)
			pastAdv = "曾經"
		monthWd = "{}個小時".format(f['pnl_prd'])
...
def generate_comment_pppscf(f,dotSign='.',prcn=0,usdSign='$ ',udfLst=None,lang="en",ts=None):
	...
	past_difday=difday
	past_act_chg_pct=roundPct(act_chg,n=0,pct=100,dotSign=dotSign,lang=lang)
	past_range=str(delta2dates(int(f.iloc[-1]['pbdate']),int(f.iloc[0]['pbdate']),fq='M')) + ("-hour" if lang=="en" else "小時")
...
def run_comment_ohlc(aX,enhanceX=None,tempS=None,dotSign='.',prcn=0,usdSign='$',lang="cn"):
	...
	if enhanceX is not None:
		...
		enhanceClause= generate_comment_ohlc(enhanceX,tempE,dotSign=dotSign,lang=lang)
		aX['enhanceClause']=enhanceClause
	else:
		aX['enhanceClause']=""

	ret=generate_comment_ohlc(aX,tempX,dotSign=dotSign,lang=lang)
	aX['nsig']=aX['tsig']
	aX['comment_ohlc']=ret
	return aX
...
def getdb_ohlc_minute(ticker,startYmd,endYmd,sector,label,pgDB):
	""" get info from minute_hist and minute_pnl for ohlc strategy info
	"""
	sq1="SELECT * FROM minute_latest_macd WHERE ticker = {0!r} order by pbdate DESC"
	sq2="SELECT * FROM minute_ls_signal WHERE ticker = {0!r} order by pbdate DESC"
	#sq1="SELECT ticker,pbdate,price,ma5,ma20,price-ma5 as ma5trend,signal_value_macd,signal_buysell_macd FROM minute_hist WHERE ticker = {0!r} and pbdate >= (SELECT max(pbdate) FROM minute_hist WHERE ticker = {0!r} and signal_buysell_macd <> 0 ) order by pbdate DESC"
	#sq2="select distinct * from minute_pnl where ticker={0!r} and sig=1 and lsc<>'combo' and name similar to '%%(star|harami|MACD)' order by name,lsc,pbdate DESC"
	...
	pnl_prd=delta2dates(endYmd,startYmd,fq="HOUR",rounding=1) # calc hours
	pbdate=endYmd
	try:
		...
		if len(g1)>1:
			m1['curr_deriv'] = chk_sign(g1['ma5trend'][0],g1['ma5trend'][1])
		m1['curr_date']=pbdate
		m1['pnl_prd']=pnl_prd
		m1['sector']=sector
		m1['label']=label
...
def run_comment_pricing(ticker='AAPL',label='',pgDB=None,dotSign='點',prcn=2,usdSign='元',lang="cn",mp3YN=False,ts_pricing=None,ts_title=None):
	...
	try:
		...
		print  >> sys.stderr, "comment_pricing:",comment_pricing
		print  >> sys.stderr, "comment_title:",comment_title
	except Exception,e:
		print  >> sys.stderr, "**ERROR @ run_comment_pricing():",str(e)
		return None
	return (comment_pricing,comment_title)

def run_comment_pppscf(ticker='AAPL',label='',pgDB=None,dotSign='.',prcn=0,usdSign='$',lang="cn",fp=None,ts=None):
	if fp is None:
		sqx="SELECT * FROM minute_pppscf WHERE ticker={0!r} ORDER BY pbdate"
	...
	try:
		...
		print  >> sys.stderr, "**ERROR @ run_comment_pppscf():",str(e)
		#print  >> sys.stderr, fp
		return None
	return ret
...
def generate_cmt(f,ts=None,dotSign='.',prcn=0,usdSign='$',udfLst=None,lang="en",mp3YN=False,funcname=''):
	try:
		...
		print  >> sys.stderr, "**ERROR @ {}():{}".format("generate_cmt",str(e))
		xcmt=''
	return xcmt
...
def run_comment_fcst(ticker='AAPL',label='',pgDB=None,dotSign='.',prcn=0,usdSign='',lang="cn",fp=None,ts=None,mp3YN=True):
	if fp is None:
		...
		sqx="SELECT * FROM minute_fcs WHERE ticker={0!r} and freq='W'"
		try:
			fp=getdb_ticker(ticker,sqx,pgDB).iloc[0]
		except:
			print >>sys.stderr, "No forecast available!"
			return ''
	if 'label' not in fp:
		fp['label']=label
	if 'category' not in fp:
...
def iteritem_comment_ohlc(j,tempS,tkX,mndt,mxdt,sector,label,daily_hdr,tablename,pgDB,mgDB,fp,wmode="replace",lang="cn",saveDB=True,mp3YN=False,dirname=None,debugTF=False):
	...
	print >> sys.stderr,"------",j,tkX,mndt,mxdt,sector,label
	try:
		(comment_pricing,comment_title)=run_comment_pricing(tkX,label,pgDB,dotSign=dotSign,lang=lang,mp3YN=mp3YN)
	except Exception,e:
		print  >> sys.stderr, "**ERROR {}. {} @ {}: {}".format(j,tkX,"run_comment_pricing",str(e))
		comment_pricing=''
		comment_title=''
	try:
		comment_pppscf=run_comment_pppscf(tkX,label,pgDB,dotSign=dotSign,lang=lang)
		print >>sys.stderr, "{0}:{1}".format("comment_pppscf",comment_pppscf) 
	except Exception,e:
		print  >> sys.stderr, "**ERROR {}. {} @ {}: {}".format(j,tkX,"run_comment_pppscf",str(e))
		comment_pppscf=''
	try:
		comment_fcst=run_comment_fcst(tkX,label,pgDB,dotSign=dotSign,lang=lang,mp3YN=mp3YN)
		print >>sys.stderr, "{0}:{1}".format("comment_fcst",comment_fcst) 
	except Exception,e:
		print  >> sys.stderr, "**ERROR {}. {} @ {}: {}".format(j,tkX,"run_comment_fcst",str(e))
		comment_fcst=''
	try:
		(pbdate,m1,m2)=getdb_ohlc_minute(str(tkX),mndt,mxdt,sector,label,pgDB)
		aX=run_comment_ohlc(m1,m2,tempS,dotSign=dotSign,lang=lang)
		print >>sys.stderr, "{0}:{1}".format("comment_ohlc",aX['comment_ohlc']) 
		# print >> sys.stderr,aX
	except Exception,e:
		print  >> sys.stderr, "**ERROR {}. {} @ {}: {}".format(j,tkX,"run_comment_ohlc",str(e))
		return None
	...
	aX['comment']=comment
	print >>sys.stderr, "{0}:\n{1}".format("COMMENT",comment)
	print >>sys.stdout, "{}".format(comment)
	if debugTF:
		print >> sys.stderr, aX
	...
	if saveDB is True:
		...
		save2mgdb_comment_ohlc(tmpx,mgDB,tablename)
		if debugTF is True:
			print >> sys.stderr, "save to [{}]".format(tablename)
	else:
		if debugTF is True:
			print >> sys.stderr, "no save to table:[{}]".format(tablename)
...
def batch_comment_ohlc(tkLst=None,**kwargs):
	...
	#====OHLC Comment===========================
	print >>sys.stderr, "====OHLC Comment==========================="
	if tablename is None:
		tablename="{}_{}".format("minute_comment_ohlc",lang)
		if mp3YN is True:
	...
	if tkLst is not None:
		...
		if dx.empty is True:
			print  >> sys.stderr,"{} not found!".format(tkLst)
			return None	
		else:
			dataM = dx.reset_index(drop=True)

	print  >> sys.stderr,"START batch_comment_ohlc-------------------------------------------"
	for j, tRw in dataM.iterrows():
		(tkX,mndt,mxdt,sector,label)=tRw.values
		iteritem_comment_ohlc(j,tempS,tkX,mndt,mxdt,sector,label,daily_hdr,tablename,pgDB,mgDB,fp,wmode=wmode,lang=lang,saveDB=saveDB,mp3YN=mp3YN,dirname=dirname,debugTF=debugTF)
		wmode='append'
	#fp.close()
	print  >> sys.stderr,"END batch_comment_ohlc-------------------------------------------"
...
if __name__ == "__main__" :
	(options, args)=opt_lsi_minute(sys.argv)
	tkLst=None
	if len(args)>0:
		if args[0]=='-':
	...
	batch_comment_ohlc(tkLst,**options)
