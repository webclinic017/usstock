...
def popenCall(xcmd=None,xin=None,bufsize=1024,shell=True,debugTF=False):
	...
	pin= xin if xin is None else PIPE
	p = Popen(xcmd,shell=shell,bufsize=bufsize,stdin=pin,stdout=PIPE,stderr=PIPE)
	if xin:
		p.stdin.write(xin)
	o, e = p.communicate()
	if xin:
...
def get_funcLst(unLst = ['sysCall','popenCall','udfStr_DEPRECATED']):
	henv={ k:v for (k,v) in globals().items() if hasattr(v,'__call__') and k not in unLst }
	return sorted(henv.keys())

def get_functions():
	tpLst = (types.BuiltinFunctionType,types.FunctionType)
	genv = { k:v for (k,v) in globals().items() if isinstance(v,tpLst) is True }
	unLst = ['sysCall','popenCall','udfStr_DEPRECATED']
	henv = {ky:va for ky,va in genv.items() if ky not in unLst }
	return sorted(henv.keys())
...
def jj_fmt(ts,dd={},dirname='.',fileTF=False,debugTF=False,j2onlyTF=False,**opts):
	...
	try:
		...
		tpLst = (types.BuiltinFunctionType,types.FunctionType)
		genv = { k:v for (k,v) in globals().items() if isinstance(v,tpLst) is True }
		# Security breach: no system call function allowed, 
		unLst = ['sysCall','popenCall','udfStr_DEPRECATED']
		henv = {ky:va for ky,va in genv.items() if ky not in unLst }
...
def udfWord(r,wordLst=None,thd=None,lang='cn',openTF=True,sortTF=False,ascendingTF=False,scale=1):
	...
	Note:
	  ...
	  default to [1.96,1.15,0.32,0.1,-0.1,-0.32,-1.15,-1.96] multiplied by 'scale'
	Example: udfWord(1.4,lang='cn')
	"""
	if thd is None:
		thd = [1.96,1.15,0.32,0.1,-0.1,-0.32,-1.15,-1.96]
		if scale != 1:
...
上升|攀升|上漲
小幅上漲|微漲|微微攀升|小幅微升
上下微微波動|輕微震盪|區間微微震盪，無什變動|稍稍波動|沒什麼變動
