...
from _alan_str import lsi2nlg_calc,combine_cmd2dict,jj_fmt,sysCall,popenCall,find_mdb,write2mdb
...
def show_page(pathname='index',request=None,**optx):
	...
	try:
		...
		#----------------------------------------------------------------
		if funcName=='page_api':
			# Run 'page_api' command
			dd = page_api(**opts)
			if htmlName is None:
				return jsonify(dd) if not isinstance(dd,str) else dd
	...
	if isinstance(data,dict):
		...
		for j in range(len(data)):
			...
			for x,y in data[j].items():
				data[j][x]=dtCvt(y)
		dd=data
	elif isinstance(data,pd.DataFrame) and dfTF is False:
		dd=data.to_dict(orient='records')
	else:
		dd=data
	pqint("===page_api type_data:{},type_dd:{}, OUTPUT_dd:\n{}".format(type(data),type(dd),dd)[:100], "\n",file=sys.stderr)
	return dd
...
def wrap_alanapi(**opts):
	try:
		...
		sys.stderr.write(" --alanapi OUTPUT:\n{}\n".format(data)[:80]+"\n")
	except Exception as e:
		data=dict(err=str(e))
...
def topic_theme_majorplayer(**opts):
	...
	colX = ['ticker','close','marketCap',
		'changePercent', 'CurrentShares','percentPos','SharesChangePercent',
		'fiftyTwoWeekRange','pbdate','shortName',
		'fund','funddate','pbdt']
