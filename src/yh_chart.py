...
def yh_spark_hist(tkLst=None,**optx):
	d = yh_batchSpark(tkLst=tkLst,**optx)
	return d
...
def chart2df(jdLst=[],**optx):
	...
	dx.loc[:,'change']=dx['close']-dx['xclose']
	dx.loc[:,'pchg']=dx['change']/dx['xclose']
	...
	for j,tkM in enumerate(chunkLst):
		...
		if types in ['spark','chart']:
			...
			if saveDB is True and len(jdQC)>0:
				if len(t2)>0:
					qc,dbM,erq=upsert_mdb(jdQC,clientM=dbM,tablename=t2,zpk=t2zpk,**optx)
					sys.stderr.write("=== {} of {} saved to {}\n".format(tkM,jdQC[-1],t2))
				if isEoD is True:
